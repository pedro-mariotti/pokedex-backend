import { getEvolutionChainById } from '../controller/evolution.controller.js';

/**
 * Handler serverless para a rota de cadeias de evolução.
...
export const handler = async (event) => {
    ...
    const path = event.path; // e.g., "/api/evolution-chain/123"
    const pathParameters = event.pathParameters || {}; // e.g., { chainId: "123" }
    const queryStringParameters = event.queryStringParameters || {};
    // Não esperamos corpo para uma requisição GET de evolução
    ...
    try {
        ...
        if (httpMethod === 'GET' && pathParameters.chainId && path.startsWith('/api/evolution-chain/')) {
            await getEvolutionChainById(mockReq, mockRes);
        } else {
            statusCode = 404;
            responsePayload = { error: `Route not found or method not allowed for ${httpMethod} ${path}` };
        }
    } catch (error) {
        console.error(`Error in evolution.handler for ${httpMethod} ${path}:`, error);
